...
- [ ] Настройка Supabase проекта (опционально)
...
- [ ] Локальная БД (Hive/SQLite/Isar)
- [ ] Роутинг (go_router/auto_route)
- [ ] DI (get_it/riverpod)
- [ ] UI библиотеки и дизайн-система

---
### Фаза 3: MVP — Базовая функциональность

> **Scope MVP**: Weekly Board + голосовое наполнение чеклиста через AI + локальное хранение данных
...
#### 3.2 Голосовое наполнение чеклиста через AI
...
- [ ] Свайп между днями/неделями
- [ ] Верхняя панель навигации

---

#### 3.4 Создание и редактирование чеклиста (базовая версия)

**Задачи**:
- [ ] Форма создания нового чеклиста
- [ ] Добавление участников семьи (имя, аватар, цвет)
- [ ] Добавление задач вручную
- [ ] Интеграция голосового ввода задач
- [ ] Назначение задач участникам
- [ ] Настройка периодичности (ежедневно/по дням/N раз в неделю)
- [ ] Сохранение чеклиста в локальную БД
- [ ] Редактирование существующего чеклиста

---

#### 3.5 Dashboard (упрощённая версия для MVP)
...
**Результат Фазы 3**: Работающий MVP с Weekly Board, голосовым вводом задач, локальным хранением данных.
...
#### 4.3 Синхронизация между устройствами (CloudKit/Supabase)

**Задачи**:
- [ ] Анализ: можно ли сделать без бэкенда (оценка CloudKit для cross-device sync)
- [ ] Если CloudKit недостаточно → настройка Supabase
  - [ ] Схема БД в Supabase
  - [ ] API для синхронизации данных
  - [ ] Авторизация (Apple Sign In + Supabase Auth)
...
#### 4.4 Многопользовательский доступ и приглашения
